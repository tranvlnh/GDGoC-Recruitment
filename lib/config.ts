...
export const questions = questionsSchema.parse(rawQuestions).sort((a, b) => a.order - b.order);
...
export function getApplicationWindowStatus(now = new Date()) {
  ...
  if (now < openAt) return { isOpen: false, reason: "not_opened" as const, openAt, closeAt };
  if (now > closeAt) return { isOpen: false, reason: "closed" as const, openAt, closeAt };
...
export function departmentLabel(departmentId: string) {
  return departments.find((department) => department.id === departmentId)?.label ?? departmentId;
}
